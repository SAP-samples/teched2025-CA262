...
As we have a Fiori Elements app with almost no custom code, the reduction is obviously not huge, but still we can see how it works. Two additional files will be generated: a `<file_name>-dbg.js` file with the original content and a `<file_name>.map` file that maps both files and will be used by the developer tools to show the source code while debugging.
